document.addEventListener("DOMContentLoaded", function () {
    ...
    async function fetchApplicantType() {
        ...
        if (memberFilter.options && typeof memberFilter.options.length !== 'undefined') {
            while (memberFilter.options.length > 1) {
                memberFilter.remove(1);
            }
        } else {
            console.log("memberFilter.options is undefined or has no length property");
            // Maybe initialize it or handle this case
        }
        ...
        try {
            ...
            if (data && data.data) {
                applicantTypes = data.data; // Store this globally
            } else {
                console.log("API response doesn't contain expected data structure:", data);
                applicantTypes = []; // Initialize as empty array to prevent errors
            }
          
            console.log("Fetched applicant types:", applicantTypes);
/*
        if (memberFilter) {
            ...
            // Add applicant types to dropdown only if we have any
            if (Array.isArray(applicantTypes) && applicantTypes.length > 0) {
                applicantTypes.forEach(type => {
                    // Check if type exists and has the property we need
                    if (type && (type.designation_of_applicant || type.Designation_of_Applicant)) {
                        const option = document.createElement("option");
                        option.value = type.designation_of_applicant || type.Designation_of_Applicant;
                        option.textContent = type.designation_of_applicant || type.Designation_of_Applicant;
    ...
    function openFilterModal() {
        ...
        // Check if applicantTypes is empty and fetch if needed
        if (!applicantTypes || applicantTypes.length === 0) {
            console.log("applicantTypes is empty, fetching again...");
            fetchApplicantType().then(() => {
                // After fetching, populate the modal with the updated data
                populateFilterModal(filterModal);
            });
        } else {
            populateFilterModal(filterModal);
        }
    ...
    function populateFilterModal(filterModal) {
        if (!filterModal) {
            ...
            title.textContent = '选择种类筛选';
            
            // Add description
            const description = document.createElement('p');
            description.textContent = '请选择要筛选的塾员种类类型';
            ...
            console.log("About to add applicant types to modal:", applicantTypes);
            
            // Add applicant type options
            if (applicantTypes && applicantTypes.length > 0) {
                applicantTypes.forEach(item => {
                    console.log("Processing item:", item);
                    
                    // Try to find the right property
                    const designationValue = item.designation_of_applicant || 
                                            item.Designation_of_Applicant || 
                                            item.designationOfApplicant;
                    
                    console.log("Found designation value:", designationValue);
                    ...
                    if (designationValue) {
                        ...
                        option.onclick = function() {
                            selectFilterOption(designationValue);
                        };
                ...
                noOptions.textContent = '没有可用的筛选选项';
      ...
      // Add function to close the filter modal
    ...
    // Add function to handle filter option selection
    function selectFilterOption(value) {
        ...
        if (memberFilter) {
            memberFilter.innerHTML = value ? 
                `<i class="fas fa-filter"></i> ${value}` : 
                `<i class="fas fa-filter"></i> 选择种类`;
        }
    ...
    function initializeEventListeners() {
        ...
        memberFilter?.addEventListener('click', function(e) {
            e.preventDefault(); // Prevent default dropdown behavior if it's a select element
            openFilterModal();
        });
